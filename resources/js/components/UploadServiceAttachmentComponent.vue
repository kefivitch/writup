      ...
      dropzoneOptions: {
        ...
        init: function() {
          ...
          this.on("addedfile", function(file) {
            var li_count = jQuery(".wt-attachmentsholder").find(
              ".lara-attachment-files .form-group ul li"
            ).length;
            count = li_count - 1;
            jQuery("#" + myDropzone.element.id)
              .parents(".wt-attachmentsholder")
              .find(
                ".lara-attachment-files .input-preview ul.dropzone-previews"
              )
              .append(
                '<input type="hidden" value="' +
                  ...
                  '" id="hidden-' +
                  count +
                  '" class="hidden-file" name="attachments[' +
                  [count] +
                  ']">'
              );
            count++;
  ...
  methods: {
    ...
    failed: function(file, message, xhr) {
      if (file.type != this.$refs.success_ref.options.acceptedFiles) {
        ...
        var input_hidden_id = jQuery("#" + media_id)
          .parents(".wt-attachmentsholder")
          .find(
            ".lara-attachment-files .input-preview ul.dropzone-previews input[type=hidden]"
          )
          .attr("id");
        if (input_hidden_id) {
