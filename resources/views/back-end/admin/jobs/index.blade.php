                    ...
                    <fieldset>
                        <div class="form-group">
                            <input type="text" name="keyword"
                                value="{{{ !empty($_GET['keyword']) ? $_GET['keyword'] : '' }}}" class="form-control"
                                placeholder="{{{ trans('lang.ph_search_jobs') }}}">
                        ...
                        <div class="wt-managejobcontent">
                            ...
                            <div class="wt-userlistinghold wt-featured wt-userlistingvtwo del-job-{{ $job->id }}">
                                ...
                                <div class="wt-userlistingcontent">
                                    <div class="wt-contenthead">
                                        ...
                                        @endif
                                        @if (!empty($job->price) || !empty($location['title']) ||
                                    ...
                                    <div class="wt-rightarea la-pending-jobs">
                                        <div class="wt-btnarea">
                                            <a href="{{{ url('job/edit-job/'.$job->slug) }}}"
                                                class="wt-btn">{{ trans('lang.edit_job') }}</a>
                                            <a href="javascript:void(0);" v-on:click.prevent="deleteJob({{$job->id}})"
                                                class="wt-btn">{{ trans('lang.del_job') }}</a>
                                            @if (!empty($cancel_proposal) &&
                                            Helper::getOrderPayout($cancel_proposal->id)->count() == 0)
                                            <a href="javascript:void(0);"
                                                v-on:click.prevent="showRefoundForm({{ $job->id }})"
                                                class="wt-btn"><span>{{ trans('lang.refund_now') }}</span></a>
                                            @endif
                                        </div>
                                        ...
                                        <div class="wt-hireduserstatus">
                                            ...
                                            <h4>{{{ $job->status }}}</h4>
                                            <a href="{{{ url('proposal/'.$job->slug . '/'. $job->proposals[0]->status) }}}"
                                            ...
                                            <a href="{{{ url('proposal/'.$job->slug . '/cancelled') }}}"
                                                class="wt-btn">{{ trans('lang.view_detail') }}</a>
                            ...
                            <b-modal ref="myModalRef-{{ $job->id }}" hide-footer title="Refund" v-cloak>
                                <div class="d-block text-center">
                                    {!! Form::open(['url' => '', 'class' =>'wt-formtheme', 'id' =>
                                    'submit_refund_'.$job->id,
                                    '@submit.prevent'=>'submitRefund("'.$job->id.'")'])
                                    !!}
                                    <fieldset>
                                        <div class="form-group">
                                            <span class="wt-select">
                                                <select id="refundable_user_id-{{$job->id}}" class="form-control"
                                                    placeholder="{{ trans('lang.select_users') }}"
                                                    v-model="refundable_user">
                                                    <option value="">{{ trans('lang.select_users') }}</option>
                                                    <option value="{{$job->employer->id}}">
                                                        {{ trans('lang.search_filter_list.employers_val') }}</option>
                                                    @if (!empty($cancel_proposal))
                                                    <option value="{{$cancel_proposal->freelancer_id}}">
                                                        {{ trans('lang.search_filter_list.freelancer_val') }}</option>
                                                    @endif
                                                </select>
                                            </span>
                                        </div>
                                        @if (!empty($cancel_proposal))
                                        <input type="hidden" value="{{$cancel_proposal->amount}}"
                                            id="refundable-amount-{{$job->id}}">
                                        <input type="hidden" value="{{$cancel_proposal->id}}"
                                            id="refundable-order-id-{{$job->id}}">
                                        @endif
